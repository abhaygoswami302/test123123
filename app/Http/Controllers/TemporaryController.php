...
class TemporaryController extends Controller
{
    ...
    {
        $temporaries = Temporary::where('user_id', '=', Auth::user()->id)->latest()->get();
        return view('user.temp.index', compact('temporaries'));
    }
        /**
...
dd($import->errors());*/
      /*  try {
            Excel::import(new TemporaryImport,request()->file('file'));
         }
    ...
    public function tempShow($id)
    {
        ...
        return view('user.temp.show', compact('temp', 'categories', 'subseries', 'themes'));
    }

    public function tempEdit($id, Request $request)
    {
        ...
        $temporary->series = $subseries->series_name;
        $temporary->casting_name = $request->casting_name;
        ...
        return redirect()->route('tempIndex')->with('tempEdit', 'Product Edited Successfully');
    }
    ...
    {
        $temporaries = Temporary::where('user_id', '=', Auth::user()->id)->orderBy('created_at', 'DESC')->get();
        ...
        return redirect()->back()->with('productsLoaded', 'Products Uploaded Successfully');
    }
}
